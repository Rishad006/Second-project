...
<form action="rtn.html" method="post">
...
<form action="generatetimetable.php" method="post">
    <div align="center" style="margin-top: 20px">
        <select name="select_semester" class="list-group-item">
            ...
            <option value="1">Batch-62</option>
            <option value="2">Batch-61</option>
            <option value="3">Batch-60</option>
            <option value="4">Batch-59</option>
            <option value="5">Batch-58</option>
            <option value="6">Batch-57</option>
            <option value="7">Batch-56</option>
            <option value="8">Batch-55</option>
        </select>
...
<div>
    ...
    <div id="TT" style="background-color: #FFFFFF">
        <table border="4" cellspacing="6" align="center" id="timetable">
            ...
            <tr>
            <td style="text-align:center; background:orange; color:white">WEEKDAYS</td>
                <td style="text-align:center; background:orange; color:white">9:00-9:50</td>
                <td style="text-align:center; background:orange; color:white">9:50-10:40</td>
                <td style="text-align:center; background:orange; color:white">10:40-11:30</td>
                <td style="text-align:cente; background:orange; color:white">11:30-12:20</td>
                <td style="text-align:center;background:orange; color:white">12:20-1:10</td>
                <td style="text-align:center; background:orange; color:white">1:10-2:00</td>
                <td style="text-align:center; background:orange; color:white">2:00-2:50</td>
                <td style="text-align:cente; background:orange; color:white">2:50-3:40</td>
                <td style="text-align:center; background:orange; color:white">3:40-4:30</td>
                <td style="text-align:center; background:orange; color:white">4:30-5:20</td>
            </tr>
            <tr>
                <?php
                $table = null;
                ...
                if (isset($_POST['select_semester']) || isset($_POST['select_teacher']) || isset($_GET['display'])) {
                    ...
                    while ($row = mysqli_fetch_assoc($q)) {
                        ...
                        echo "
                        <tr><td style=\"text-align:center; background:green;color:white\">$days[$i]</td>
                        <td style=\"text-align:center; background:khaki\">{$row['period1']}</td>
                       <td style=\"text-align:center; background:khaki\">{$row['period2']}</td>
                       <td style=\"text-align:center; background:khaki\">{$row['period3']}</td>
                        <td style=\"text-align:center; background:khaki\">{$row['period4']}</td>
                         <td style=\"text-align:center; background:khaki\">{$row['period5']}</td>
                         <td style=\"text-align:center; background:khaki\">{$row['period6']}</td>
                         <td style=\"text-align:center; background:khaki\">{$row['period7']}</td>
                         <td style=\"text-align:center; background:khaki\">{$row['period8']}</td>
                         <td style=\"text-align:center; background:khaki\">{$row['period9']}</td>
                         <td style=\"text-align:center; background:khaki\">{$row['period9']}</td>
                </tr>\n";
                    }
...
<div align="center" style="margin-top: 10px">
    <button id="saveaspdf" class="btn btn-info btn-lg" onclick="downloadPDF()">SAVE AS PDF</button>
</div>
...
<script>
    function downloadPDF() {
        ...
        html2canvas(element, {
            ...
            var imgData = canvas.toDataURL('image/jpeg', 1.0); // Use JPEG format
            var pdf = new jsPDF('p', 'mm', 'a4'); // Portrait orientation, A4 size
            pdf.addImage(imgData, 'JPEG', 0, 0, pdf.internal.pageSize.getWidth(), pdf.internal.pageSize.getHeight());
            pdf.save('timetable.pdf'); // Download the PDF
            alert("Timetable saved as PDF!");
        });
    }

    document.getElementById('timetableForm').onsubmit = function(event) {
